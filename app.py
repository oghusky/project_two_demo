...
clean_pokemon = pd.read_csv(
    './resources/cleaned_pokemon.csv').to_dict('records')
...
# post, put(grab user by username and update the username), delete(grab by ID and delete)
...
def sql_data():
    df = pd.read_sql('''SELECT * FROM pokedex''', con=engine)
